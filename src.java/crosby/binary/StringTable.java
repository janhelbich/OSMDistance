...
public class StringTable {
	...
	/**
	 * After the stringtable has been built, return the offset of a string in
	 * it.
	 *
	...
	public void finish() {
		 ...
		 * 0 : Is reserved (used as a delimiter in tags A: 1 to 127 : Uses can
		 * be represented with 1 byte B: 128 to 128**2-1 : Uses can be
		 ...
		 * better entropy encoding out of deflate) 3. I want the stringtable to
		 * compress as small as possible.
		 * 
		 * Condition 1 is obvious. Condition 2 makes deflate compress
		 * stringtable references more effectively. When compressing entities,
		 * delta coding causes small positive integers to occur more frequently
		 * than larger integers. Even though a stringtable references to indices
		 * of 1 and 127 both use one byte in a decompressed file, the small
		 * integer bias causes deflate to use fewer bits to represent the
		 * smaller index when compressed. Condition 3 is most effective when
		 * adjacent strings in the stringtable have a lot of common substrings.
		 * 
		 * So, when I decide on the master stringtable to use, I put the 127
		 ...
		 * sort them lexiconographically, to maximize goal 3 and ignoring goal
		 * 2.
		 * 
		...
		for (int i = 0; i < set.length; i++) {
			stringmap.put(set[i], new Integer(i + 1)); // Index 0 is reserved
														// for use as a
														// delimiter.
		}
		counts = null;
	...
	public Osmformat.StringTable.Builder serialize() {
		Osmformat.StringTable.Builder builder = Osmformat.StringTable
				.newBuilder();
		builder.addS(ByteString.copyFromUtf8("")); // Add a unused string at
													// offset 0 which is used as
													// a delimiter.
		for (int i = 0; i < set.length; i++)
